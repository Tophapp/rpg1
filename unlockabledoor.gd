...
func _input(event):
	if event.is_action_pressed("done") and player_in_range and not key in $"../../characters/player".keys:
		$"../../characters/player".popup('A keyhole with the words, "This Door Reqires Key Number '+str(key)+' To Unlock" carved into the stone.')
	elif event.is_action_pressed("done") and player_in_range and key in $"../../characters/player".keys:
		$"../../characters/player".popup('You push key number '+str(key)+" into the slot, and the door unlocks.")
