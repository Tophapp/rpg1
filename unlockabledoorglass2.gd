...
func _input(event):
	if event.is_action_pressed("done") and player_in_range and not ($"..".blood):
		$"../../characters/player".popup('You do not have royal blood')
	elif event.is_action_pressed("done") and player_in_range and ($"..".blood):
		$"../../characters/player".popup("The door yawns awake slowly")
